...
namespace Stock_Management
{
    public partial class Providers : Form
    {
        ...
        private bool ifProviderExists(SqlConnection con, string providerCode)
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select 1 From [Providers] WHERE [ProviderCode] = '" + providerCode + "' ", con);
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            if (Validation())
            {
                ...
                else
                {
                    ...
                    sqlQueryUpdateQuantity = @"UPDATE [Products] SET [Quantity] += '" + productQuantityTextBox.Text + "' FROM [Products], [Providers] WHERE Products.ProductCode = '" + productCodeTextBox.Text + "'";
                }
        ...
        public void LoadData()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("Select * From [dbo].[Providers]", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
        ...
        private void providerCompanyNameTextBox_TextChanged(object sender, EventArgs e)
        {
            if (providerCompanyNameTextBox.Text.Length > 0)
            {
                ...
                this.dgview.MouseDoubleClick += new System.Windows.Forms.MouseEventHandler(this.providerCode_MouseDoubleClick);
                SqlConnection con = Connection.GetConnection();
                SqlDataAdapter sda = new SqlDataAdapter("Select Top(10) ProviderCode,ProviderName From [Providers] WHERE [ProviderName] Like '" + providerCompanyNameTextBox.Text + "%'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                dgview.Rows.Clear();
                foreach (DataRow row in dt.Rows)
        ...
        private void providerCode_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                providerCompanyNameTextBox.Text = dgview.SelectedRows[0].Cells[1].Value.ToString();
                SqlCommand comm = new SqlCommand("SELECT * FROM [Providers] WHERE (ProviderName = '" + providerCompanyNameTextBox.Text + "')", con);
