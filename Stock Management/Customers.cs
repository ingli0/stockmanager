...
namespace Stock_Management
{
    public partial class Customers : Form
    {
        ...
        private bool ifCustomerExists(SqlConnection con, string customerCode)
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select 1 From [Customers] WHERE [CustomerCode] = '" + customerCode + "' ", con);
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            if (Validation())
            {
                ...
                else
                {
                    ...
                    sqlQueryUpdateQuantity = @"UPDATE [Products] SET [Quantity] -= '" + productQuantityTextBox.Text + "' FROM [Products], [Customers] WHERE Products.ProductCode = '" + productCodeTextBox.Text + "'";
                    sqlQueryUpdateSalesFrequency = @"UPDATE [Products] SET [SalesFrequency] += '" + productQuantityTextBox.Text + "' FROM [Products], [Customers] WHERE Products.ProductCode = '" + productCodeTextBox.Text + "'";
                }

                SqlCommand cmd = new SqlCommand(sqlQuery, con);
                SqlCommand cmdUpdateQuantity = new SqlCommand(sqlQueryUpdateQuantity, con);
                SqlCommand cmdUpdateSalesFrequency = new SqlCommand(sqlQueryUpdateSalesFrequency, con);
                cmd.ExecuteNonQuery();
        ...
        public void LoadData()
        {
            ...
            SqlDataAdapter sda = new SqlDataAdapter("Select * From [dbo].[Customers]", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
        ...
        private void customerCompanyNameTextBox_TextChanged(object sender, EventArgs e)
        {
            if (customerCompanyNameTextBox.Text.Length > 0)
            {
                this.dgview.Visible = true;
                dgview.BringToFront();
                Search(33, 137, 238, 90, "Cust Code,Cust Name", "100,0");
                this.dgview.MouseDoubleClick += new System.Windows.Forms.MouseEventHandler(this.customerCode_MouseDoubleClick);
                SqlConnection con = Connection.GetConnection();
                SqlDataAdapter sda = new SqlDataAdapter("Select Top(10) CustomerCode,CustomerName From [Customers] WHERE [CustomerName] Like '" + customerCompanyNameTextBox.Text + "%'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                dgview.Rows.Clear();
                foreach (DataRow row in dt.Rows)
        ...
        private void customerCode_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                customerCompanyNameTextBox.Text = dgview.SelectedRows[0].Cells[1].Value.ToString();
                SqlCommand comm = new SqlCommand("SELECT * FROM [Customers] WHERE (CustomerName = '" + customerCompanyNameTextBox.Text + "')", con);
