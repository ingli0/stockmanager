    ...
    public partial class Products : Form
    {
        ...
        private void productNameTextBox_TextChanged(object sender, EventArgs e)
        {
            if (productNameTextBox.Text.Length > 0)
            {
                this.dgview.Visible = true;
                dgview.BringToFront();
                Search(37, 122, 363, 90, "Prod Code,Prod Name,Prod Cat", "116,125,0");
                this.dgview.MouseDoubleClick += new System.Windows.Forms.MouseEventHandler(this.proCode_MouseDoubleClick);
                SqlConnection con = Connection.GetConnection();
                SqlDataAdapter sda = new SqlDataAdapter("Select Top(10) ProductCode,ProductName,Category From [Products] WHERE [ProductName] Like '" + productNameTextBox.Text + "%'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                dgview.Rows.Clear();
                foreach (DataRow row in dt.Rows)
        ...
        private void productCategoryTextBox_TextChanged(object sender, EventArgs e)
        {
            if (productCategoryTextBox.Text.Length > 0)
            {
                ...
                this.dgview.MouseDoubleClick += new System.Windows.Forms.MouseEventHandler(this.proCode_MouseDoubleClick);
                SqlConnection con = Connection.GetConnection();
                SqlDataAdapter sda = new SqlDataAdapter("Select Top(10) ProductCode,ProductName,Category From [Products] WHERE [Category] Like '" + productCategoryTextBox.Text + "%'", con);
                DataTable dt = new DataTable();
                sda.Fill(dt);
                dgview.Rows.Clear();
                foreach (DataRow row in dt.Rows)
        ...
        private void proCode_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            {
                productNameTextBox.Text = dgview.SelectedRows[0].Cells[1].Value.ToString();
                SqlCommand comm = new SqlCommand("SELECT * FROM [Products] WHERE (ProductName = '" + productNameTextBox.Text + "')", con);
